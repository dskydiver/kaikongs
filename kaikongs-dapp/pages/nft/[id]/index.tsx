...
const nft = () => {
    ...
    const listSale = async () => {
        ...
        if (price % 1 == 0 && price > 0){
            try{
                ...
                const isApproved = await nftContract.methods.isApprovedForAll(wallet, contractAddress).call()
            ...
            setStatus({message: "Price can be in only whole numbers. Min. 1 KAI", type: "error"})
            setIsLoading(false);
        }
    }
    ...
    const transferToken = async () => {
        setProcessing(true);
        try {
            const tx = await nftContract.methods.transferFrom(wallet, recipient, id).send({from: wallet});
            await tx;
            location.reload()
    ...
    const fetchMetadata = async () => {
        ...
        const nft = metadataJSON.find((nft) => +nft.edition === +tokenMetadata.name.split("#")[1]);
        ...
        setImageUrl(`https://kai-kongs.myfilebase.com/ipfs/${img.split("ipfs://")[1]}`)
        ...
        return (
            ...
            <div className="container px-5 py-24 mx-auto">
                <div className="lg:w-4/5 mx-auto flex flex-wrap">
                    <img className="lg:w-1/2 w-full h-full object-cover object-center rounded" src={imageUrl}/>
                        ...
                        <div className="nft__controlls mt-6">
                            <div className='flex gap-2'>
                                    ...
                                    wallet &&   
                                        ...
                                        <div>
                                            <div className='flex gap-2'>
                                                ...
                                                <button disabled={isLoading} onClick={listSale} className='px-3 py-2 bg-blue-600 rounded-lg text-white'>{isLoading ? 'Listing...' : 'List For Sale'}</button>
                                            </div> 
                                            ...
                                            <div className="transfer__wrapper mt-8">
                                                <span className='text-lg mt-4 text-black'>Transfer Your NFT</span>
                                                <div className='flex gap-2 mt-1'>
                                                    <input disabled={isLoading} type="text" placeholder="Recipient's Address" id="price" name="price" className="bg-gray-100 bg-opacity-50 rounded-lg border border-gray-300 focus:border-indigo-500 focus:bg-transparent focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out w-[13rem]" onChange={(e) => {setRecipient(e.target.value)}}  />

                                                    <button disabled={isLoading} onClick={transferToken} className='px-3 py-2 bg-blue-600 rounded-lg text-white'>{processing ? 'Processing...' : 'Transfer'}</button>
                                                </div> 
                                            </div>
                                        </div>
                                 : <></>}
                                ...
                                {isListed && (
                                    ...
                                    wallet && 
                                        !isOwner ? (<button disabled={isLoading} onClick={buyItem} className='p-3 bg-green-600 rounded-lg text-white'>{isLoading ? 'Processing..' : 'Buy Now'}</button>)
                                        :
                                    ...
                                    <details>
                                        ...
                                        <div className="flex flex-wrap mt-3 gap-4">
                                            {nftTraits.map(attr => (
                                                ...
                                                {/* <span>{sentenceCase(attr.value)} {attr.rarity_score}% </span> */}
