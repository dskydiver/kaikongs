...
const Holdings = () => {
    ...
    const fetchNFTs = async () => {
        ...
        const fetchData = async (id) => {
            const tokenPrice = await contract.methods.getPrice(id).call()
            return tokenPrice;
        }

        const priceMap = await Promise.all(marketItems.map(fetchData))

        const combinedArray = marketItems.map((id, index) => ({ id, tokenPrice: priceMap[index] }));
        // console.log(combinedArray);

        // sort the combined array by rank
        combinedArray.sort((a, b) => b.tokenPrice - a.tokenPrice);
        ...
        for (let i = 0; i < sortedTokenIds.length; i++) {
            ...
            const tokenPrice = await contract.methods.getPrice(sortedTokenIds[i]).call();

            setNftItems(nftItems => [...nftItems, { id: nftItems.length, name: tokenName, image: tokenImage, token: sortedTokenIds[i], tokenPrice: tokenPrice, rank: nft.rank }])
        }
    }
                    ...
                    <div className='flex justify-end mb-5'>
                        ...
                        <div id="dropdown" className="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow">
                            <ul className="py-2 w-fit text-sm text-gray-700 " aria-labelledby="dropdownDefaultButton">
                                <li>
                                    <a href='/sort-rank' id="r-h-l" className="block w-full px-4 py-2 hover:bg-gray-100">Rank,  High To Low</a>

                                    <a href='/sort-price-lh' id="p-l-h" className="block w-full px-4 py-2 hover:bg-gray-100">Price, Low To High</a>

                                    <a href='/sort-price-hl' id="p-h-l" className="active__option block w-full px-4 py-2 hover:bg-gray-100">Price, High To Low</a>
